...
$(document).ready(function () {
    ...
    var vm = function () {
        ...
        self.activate = function (id) {
            ...
            else if (window.location.search.includes("SortingBy=") && window.location.search.includes('level=')) {
                var sort = (window.location.search).replace(/[?=&]/g, "");
                var sortOrganized = sort.slice(9,).replace('level', "");
                composedUri = `http://192.168.160.58/Olympics/api/Athletes?page=${id}&pagesize=25&sortby=${sortOrganized}`
                ajaxHelper(composedUri, 'GET').done(function (data) {
                    hideLoading();
